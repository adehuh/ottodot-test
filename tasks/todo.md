...
- [ ] **T1 · Repo scaffold and quality gates** — M · deps: none
...
- [ ] **T2 · `src/domain` — pure types, result codes, DTOs** — S · deps: T1
...
- [ ] **T3 · Initial migration + raw-SQL duplicate proof (B1)** — M · deps: T1, T2
...
- [ ] **T4 · `db/seed.ts` — every case the brief names** — S · deps: T3
...
- [ ] **T5 · `src/db` infrastructure — pool, errors, transaction helper** — M · deps: T2, T3
...
- [ ] **T6 · Repositories — all SQL, typed and parameterized** — M · deps: T4, T5
...
- [ ] Layer lint rule green; SQL/`BEGIN` placement greps clean

---

## Phase 2 — The graded core

- [ ] **T7 · `src/payments` — deterministic mock gateway** — S · deps: T2
...
- [ ] **T8 · `createBooking(studentId, classId)`** — M · deps: T6, T7
...
- [ ] **T9 · `confirmBooking(bookingId, simulate)` — THE GRADED CORE** — M–L · deps: T8
...
- [ ] **T10 · The race test (B4, B5)** — M · deps: T9
...
- [ ] **T11 · The lock-disabled proof (B7)** — S · deps: T10
...
- [ ] **Invariant-removal sweep (R9.6)** — each verified manually, recorded in the README:
  - [ ] drop `FOR UPDATE` → suite red
  - [ ] drop `bookings_one_live_per_student_class` → suite red
  - [ ] drop `bookings_confirmed_by_class` → suite red (via the `pg_indexes` assertion)
- [ ] `npm run verify` green
- [ ] **Review with human before proceeding to the API**
